...
produce the same results unless a column is marked ``random`` and no random seed is given.
...
column specification is marked as ``random`` and no random seed was provided.

If a random seed is provided, either as an argument to the DataGenerator instance specification,
or as option on the column specification, the random seed will be applied to all random number generation
us
