...
class TestWeights(unittest.TestCase):
    ...
    def test_get_observed_weights(self):
        alpha_desired_weights = [9, 4, 1, 10, 5, 9, 4, 1, 10, 5, 9, 4, 1, 10, 5, 9, 4, 1, 10, 5, 9, 4, 1, 10, 5, 9]
        alpha_list = list("abcdefghijklmnopqrstuvwxyz")
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("pk1", "int", unique_values=100)
            .withColumn(
                "alpha", "string", values=alpha_list, baseColumn="pk1", weights=alpha_desired_weights, random=True
            )
        )
    ...
    def test_generate_values(self):
        df_values = spark.sql(
            "select * from (select tech, count(tech) as rc from testdata group by tech ) a order by tech"
        ).collect()
    ...
    def test_weighted_distribution(self):
        ...
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("alpha", "string", values=alpha_list, weights=alpha_desired_weights, random=True)
        )
        dfAlpha = dsAlpha.build().cache()
    ...
    def test_weighted_distribution_nr(self):
        ...
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("alpha", "string", values=alpha_list, weights=alpha_desired_weights)
        )
        dfAlpha = dsAlpha.build().cache()
        observed_weights = self.get_observed_weights(dfAlpha, 'alpha', dsAlpha['alpha'].values)
    ...
    def test_weighted_distribution_nr2(self):
        ...
        dsAlpha = (
            ...
            .withColumn("alpha", "string", values=alpha_list, baseColumn="pk1", weights=alpha_desired_weights)
        )
        dfAlpha = dsAlpha.build().cache()
        observed_weights = self.get_observed_weights(dfAlpha, 'alpha', dsAlpha['alpha'].values)
    ...
    def test_weighted_distribution_nr3(self):
        ...
        # dont use seed value as non random fields should be repeatable
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4, debug=True)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("pk1", "int", unique_values=500)
            .withColumn("pk2", "int", unique_values=500)
            .withColumn("alpha", "string", values=alpha_list, baseColumn=["pk1", "pk2"], weights=alpha_desired_weights)
        )
        dfAlpha = dsAlpha.build().cache()
        observed_weights = self.get_observed_weights(dfAlpha, 'alpha', dsAlpha['alpha'].values)
    ...
    def test_weighted_distribution_int(self):
        ...
        dsInt1 = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("code", "integer", values=num_list, weights=num_desired_weights, random=True)
        )
    ...
    def test_weighted_nr_int(self):
        ...
        dsInt1 = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 10000, partitions=4, debug=True)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("code", "integer", values=num_list, weights=num_desired_weights, base_column_type="hash")
        )
        dfInt1 = dsInt1.build().cache()
    ...
    def test_weighted_repeatable_non_random(self):
        ...
        # dont use seed value as non random fields should be repeatable
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 1000, partitions=4)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("alpha", "string", values=alpha_list, weights=alpha_desired_weights)
        )
    ...
    def test_weighted_repeatable_random(self):
        ...
        dsAlpha = (
            dg.DataGenerator(sparkSession=spark, name="test_dataset1", rows=26 * 1000, partitions=4, seed=43)
            .withIdOutput()  # id column will be emitted in the output
            .withColumn("alpha", "string", values=alpha_list, weights=alpha_desired_weights, random=True)
        )
