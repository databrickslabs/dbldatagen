...
class TestStreaming(unittest.TestCase):
    ...
    def test_streaming2(self):
        time_now = int(round(time.time() * 1000))
        base_dir = "/tmp/testdatagenerator2_{}".format(time_now)
        test_dir = os.path.join(base_dir, "data2")
        checkpoint_dir = os.path.join(base_dir, "checkpoint2")
        print(time_now, test_dir, checkpoint_dir)
        ...
        try:
            ...
            testDataSpec = (dg.DataGenerator(sparkSession=spark, name="test_data_set1", rows=self.row_count,
                                             partitions=4, seedMethod='hash_fieldname')
                            .withSeedColumnOutput(outputStreamingColumns=True)
                            .withColumn("r", FloatType(), expr="floor(rand() * 350) * (86400 + 3600)",
                                        numColumns=self.column_count)
                            .withColumn("code1", IntegerType(), minValue=100, maxValue=200)
                            .withColumn("code2", IntegerType(), minValue=0, maxValue=10)
            ...
            fieldsFromSchema = set([fld.name for fld in df2.schema.fields])
        ...
        self.assertTrue("_source_timestamp" in fieldsFromSchema)
