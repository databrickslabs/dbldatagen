...
class TestQuickTests:
    ...
    def test_analyzer(self):
        testDataDF = (
            dg.DataGenerator(sparkSession=spark, name="test_data_set1", rows=1000, partitions=4).withIdOutput().build()
        )
    ...
    def test_column_specifications(self):
        tgen = (
            DataGenerator(sparkSession=spark, name="test_data_set", rows=1000000, partitions=8)
            .withSchema(schema)
            .withColumn(
                "sector_status_desc", StringType(), minValue=1, maxValue=200, step=1, prefix='status', random=True
            )
            .withColumn("s", StringType(), minValue=1, maxValue=200, step=1, prefix='status', random=True, omit=True)
        )
    ...
    def test_basic_formatting1(self):
        testDataSpec = (
            dg.DataGenerator(sparkSession=spark, name="formattedDF", rows=100000, partitions=4)
            .withIdOutput()
            .withColumn("val1", IntegerType(), unique_values=100)
            .withColumn("val2", IntegerType(), minValue=1, maxValue=100)
            .withColumn("str2", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="values")
        )
    ...
    def test_basic_formatting2(self):
        testDataSpec = (
            ...
            .withColumn("val1", IntegerType(), unique_values=100)
            .withColumn("val2", IntegerType(), minValue=1, maxValue=100)
            .withColumn("str2", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="hash")
        )
    ...
    def test_basic_formatting3(self):
        testDataSpec = (
            ...
            .withColumn("val2", IntegerType(), minValue=1, maxValue=100)
            .withColumn(
                "str5b", StringType(), format="test %s", baseColumn=["val1", "val2"], values=["one", "two", "three"]
            )
        )
    ...
    def test_basic_formatting4(self):
        testDataSpec = (
            ...
            .withColumn(
                "str5b", StringType(), format="test %s %s", baseColumn=["val1", "val2"], base_column_type="values"
            )
        )
    ...
    def test_basic_formatting5(self):
        testDataSpec = (
            ...
            .withColumn("val2", IntegerType(), minValue=1, maxValue=100)
            .withColumn(
                "str1", StringType(), format="test %s", baseColumn=["val1", "val2"], values=["one", "two", "three"]
            )
            .withColumn(
                "str2",
                StringType(),
                format="test %s",
                baseColumn=["val1", "val2"],
    ...
    def test_basic_formatting(self):
        testDataSpec = (
            ...
            .withColumn("str2", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="values")
            .withColumn("str3", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="hash")
            .withColumn("str4", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="hash")
    ...
    def test_basic_prefix(self):
        testDataSpec = (
            dg.DataGenerator(sparkSession=spark, name="formattedDF", rows=1000, partitions=4)
            .withIdOutput()
            .withColumn("val1", IntegerType(), unique_values=100)
            .withColumn("val2", IntegerType(), minValue=1, maxValue=100)
    ...
    def test_date_time_ranges(self):
        testDataSpec = (
            ...
            .withColumn(
                "last_sync_dt1", DateType(), dataRange=DateRange("2017-10-01 00:00:00", "2018-10-06 00:00:00", "days=1")
            )
            .withColumn(
                "last_sync_dt2",
                DateType(),
                dataRange=DateRange("2017-10-01 00:00:00", "2018-10-06 00:00:00", "days=1"),
                unique_values=5,
            )
            .withColumn(
                "last_sync_date",
    ...
    def test_script_merge1(self, asHtml):
        testDataSpec = (
            ...
            .withColumn("str2", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="values")
            .withColumn("str3", StringType(), format="test %s", baseColumn=["val1", "val2"], base_column_type="hash")
            .withColumn("str4", StringType(), format="test %s", baseColumn=["val1", "val2"], baseColumnType="hash")
            .withColumn("str5", StringType(), format="test %s", baseColumn=["val1", "val2"])
            .withColumn("str5a", StringType(), format="test %s", baseColumn=["val1", "val2"])
            .withColumn(
                "action", StringType(), format="test %s", baseColumn=["val1", "val2"], values=["INS", "DEL", "UPDATE"]
            )
            .withColumn("str6", StringType(), template=r"\v0 \v1", baseColumn=["val1", "val2"])
        )

        script = testDataSpec.scriptMerge(
            tgtName="Test",
            srcName="TestInc",
            joinExpr="src.id=tgt.id",
            delExpr="src.action='DEL'",
            updateExpr="src.action='UPDATE",
            asHtml=asHtml,
        )
        print(script)
    ...
    def test_strings_from_numeric_string_field1(self):
        ...
        testDataSpec = (
            dg.DataGenerator(sparkSession=spark, name="stringsFromNumbers", rows=100000, partitions=4)
            .withIdOutput()
    ...
    def test_random_generation_without_range_values(self, columnSpecOptions):
        ...
        testDataSpec = (
            dg.DataGenerator(
                sparkSession=spark, name="randomGenerationWithoutRangeValues", rows=100, partitions=4
            ).withIdOutput()
            # default column type is string
