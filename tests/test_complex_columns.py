...
class TestComplexColumns:
    ...
    @pytest.mark.parametrize(
        "complexFieldType, expectedType, invalidValueCondition",
        [
            ...
            (
                ...
                StructType(
                    [StructField("a", BinaryType()), StructField("b", IntegerType()), StructField("c", FloatType())]
                ),
    ...
    def test_uninitialized_complex_fields(self, complexFieldType, expectedType, invalidValueCondition, setupLogging):
        ...
        df_spec = (
            dg.DataGenerator(
                spark, name="test_data_set1", rows=data_rows, partitions=spark.sparkContext.defaultParallelism
            )
            .withIdOutput()
    ...
    @pytest.mark.parametrize(
        "complexFieldType, expectedType, valueInit, validCond",
        [
            ("array<int>", ArrayType(IntegerType()), "array(1,2,3)", "complex_field[1] = 2"),
            (
                "array<array<string>>",
            ...
            (
                ...
                StructType(
                    [StructField("a", StringType()), StructField("b", IntegerType()), StructField("c", FloatType())]
                ),
            ...
            (
                ...
                StructType(
                    [StructField("a", StringType()), StructField("b", IntegerType()), StructField("c", IntegerType())]
                ),
    ...
    def test_initialized_complex_fields(
        self, complexFieldType, expectedType, valueInit, validCond, setupLogging
    ...
    def test_basic_arrays_with_columns2(self, setupLogging):
        ...
        df_spec = (
            ...
            .withIdOutput()
            .withColumn(
                "r", ArrayType(FloatType()), expr="array(floor(rand() * 350) * (86400 + 3600))", numColumns=column_count
            )
        )
    ...
    def test_basic_arrays_with_columns5(self, setupLogging):
        ...
        df_spec = (
            ...
            .withIdOutput()
            .withColumn(
                "r", FloatType(), minValue=1.0, maxValue=10.0, step=0.1, numColumns=column_count, structType="array"
            )
            .withColumn("code1", IntegerType(), minValue=100, maxValue=200)
            .withColumn("code2", IntegerType(), minValue=0, maxValue=10)
            .withColumn("code3", StringType(), values=['a', 'b', 'c'])
    ...
    def test_basic_arrays_with_existing_schema(self, arraySchema, setupLogging):
        ...
        gen1 = (
            dg.DataGenerator(sparkSession=spark, name="array_schema", rows=10, partitions=2)
    ...
    def test_use_of_struct_in_schema1(self, setupLogging):
        ...
        struct_type = StructType(
            [
                ...
                StructField(
                    "city",
                    StructType([StructField('id', LongType(), True), StructField('population', LongType(), True)]),
                    True,
                ),
            ]
        )
        ...
        gen1 = (
            dg.DataGenerator(sparkSession=spark, name="nested_schema", rows=10000, partitions=4)
    ...
    def test_varying_arrays(self, setupLogging):
        df_spec = (
            ...
            .withColumn("r", "float", minValue=1.0, maxValue=10.0, step=0.1, numColumns=5)
            .withColumn(
                "observations",
                "array<float>",
    ...
    def test_single_element_array(self):
        ...
        df_spec = df_spec.withColumn("test4", "string", structType="array", values=["one", "two", "three"])
    ...
    def test_map_values(self):
        ...
        df_spec = df_spec.withColumn(
            "v4", "string", values=["this", "is", "a", "test"], numFeatures=1, structType="array"
        )
        ...
        df_spec = df_spec.withColumn(
            ...
            "map<string,string>",
            values=[
                F.map_from_arrays(F.col("v1"), F.col("v2")),
    ...
    def test_inferred_column_types_disallowed1(self, setupLogging):
        with pytest.raises(ValueError):
            ...
            data_rows = 10 * 1000
            df_spec = (
                dg.DataGenerator(spark, name="test_data_set1", rows=data_rows)
                .withIdOutput()
                .withColumn(
                    "r",
                    FloatType(),
    ...
    def test_inferred_disallowed_with_schema(self):
        ...
        testDataSpec = dg.DataGenerator(spark, name="test_data_set1", rows=10000).withSchema(schema)
    ...
    def test_inferred_column_structs2(self, setupLogging):
        ...
        assert type3 == StructType(
            [
                ...
                StructField(
                    'b', StructType([StructField('a', DateType(), False), StructField('b', StringType(), False)]), False
                ),
            ]
        )
    ...
    def test_with_struct_column2(self, setupLogging):
        ...
        type1 = self.getFieldType(df.schema, "struct1")
        assert type1 == StructType(
            [StructField('code1', IntegerType(), True), StructField('code2', IntegerType(), True)]
        )
        type2 = self.getFieldType(df.schema, "struct2")
    ...
    def test_with_struct_column4(self, setupLogging):
        ...
        df_spec = (
            ...
            .withStructColumn("struct1", fields=[('a', 'code1'), ('b', 'code2')])
            .withStructColumn("struct2", fields=[('a', 'code5'), ('b', 'code6')])
            .withStructColumn("struct3", fields={'a': {'a': 'code1', 'b': 'code2'}, 'b': {'a': 'code5', 'b': 'code6'}})
        )
