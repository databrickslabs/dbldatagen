...
class TestBasicOperation(unittest.TestCase):
    ...
    def test_alt_seed_column(self):
        ...
        self.assertTrue("_id" in fieldsFromGenerator)
        self.assertFalse("id" in fieldsFromGenerator)

        ds_copy1 = dgspec.clone()
        fieldsFromGeneratorClone = set(ds_copy1.getOutputColumnNames())

        self.assertTrue("_id" in fieldsFromGeneratorClone)
        self.assertFalse("id" in fieldsFromGeneratorClone)

    @unittest.skip("not yet implemented")
    def test_seed_column_collision1(self):
        dgspec = (dg.DataGenerator(sparkSession=spark, name="alt_data_set", rows=10000,
                                             partitions=4, seedMethod='hash_fieldname', verbose=True)
                            .withIdOutput()
    ...
    @unittest.skip("not yet implemented")
    def test_seed_column_collision1a(self):
        dgspec = (dg.DataGenerator(sparkSession=spark, name="alt_data_set", rows=10000,
                                             partitions=4, seedMethod='hash_fieldname', verbose=True)
                            .withColumn("id", FloatType(), expr="floor(rand() * 350) * (86400 + 3600)",
                                        numColumns=4)
    ...
    @unittest.skip("not yet implemented")
    def test_seed_column_collision1b(self):
        dgspec = (dg.DataGenerator(sparkSession=spark, name="alt_data_set", rows=10000,
                                             partitions=4, seedMethod='hash_fieldname', verbose=True)
                            .withColumn("id", FloatType(), expr="floor(rand() * 350) * (86400 + 3600)",
                                        numColumns=4)
