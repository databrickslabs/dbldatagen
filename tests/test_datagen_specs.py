...
class TestBasicUserDatagenSpec(unittest.TestCase):
    ...
    def test_basic_user_spec_creation(self):
        ...
        columns = [
            ColumnDefinition(name="customer_id", type="long", options={"minValue": 1000000, "maxValue": 9999999999}),
            ColumnDefinition(name="name", type="string", options={"template": r"\w \w"}),
            ColumnDefinition(name="email", type="string", options={"template": r"\w@\w.com"}),
        ]

        table_def = DatasetDefinition(number_of_rows=1000, partitions=2, columns=columns)

        spec = DatagenSpec(datasets={"users": table_def}, output_destination=None)
    ...
    def test_basic_user_spec_validation(self):
        ...
        columns = [
            ColumnDefinition(name="customer_id", type="long", options={"minValue": 1000000}),
            ColumnDefinition(name="name", type="string", options={"template": r"\w \w"}),
        ]

        table_def = DatasetDefinition(number_of_rows=100, columns=columns)

        spec = DatagenSpec(datasets={"users": table_def})
    ...
    def test_column_with_base_column(self):
        ...
        columns = [
            ColumnDefinition(name="symbol_id", type="long", options={"minValue": 1, "maxValue": 100}),
            ColumnDefinition(name="symbol", type="string", options={"expr": "concat('SYM', symbol_id)"}),
        ]

        table_def = DatasetDefinition(number_of_rows=50, columns=columns)

        spec = DatagenSpec(datasets={"symbols": table_def})
...
class TestBasicStockTickerDatagenSpec(unittest.TestCase):
    ...
    def test_basic_stock_ticker_spec_creation(self):
        ...
        columns = [
            ColumnDefinition(name="symbol", type="string", options={"template": r"\u\u\u"}),
            ColumnDefinition(
                name="post_date", type="date", options={"expr": "date_add(cast('2024-10-01' as date), floor(id / 100))"}
            ),
            ColumnDefinition(name="open", type="decimal", options={"minValue": 100.0, "maxValue": 500.0}),
            ColumnDefinition(name="close", type="decimal", options={"minValue": 100.0, "maxValue": 500.0}),
            ColumnDefinition(name="volume", type="long", options={"minValue": 100000, "maxValue": 5000000}),
        ]

        table_def = DatasetDefinition(number_of_rows=1000, partitions=2, columns=columns)

        spec = DatagenSpec(datasets={"stock_tickers": table_def}, output_destination=None)
    ...
    def test_stock_ticker_with_omitted_columns(self):
        ...
        columns = [
            ...
            ColumnDefinition(name="open", type="decimal", options={"expr": "base_price * 0.99"}),
            ColumnDefinition(name="close", type="decimal", options={"expr": "base_price * 1.01"}),
        ]

        table_def = DatasetDefinition(number_of_rows=100, columns=columns)

        spec = DatagenSpec(datasets={"prices": table_def})
...
class TestDatagenSpecValidation(unittest.TestCase):
    ...
    def test_negative_rows_validation(self):
        ...
        columns = [ColumnDefinition(name="col1", type="long")]

        # Create with negative rows using dict to bypass Pydantic validation
        table_def = DatasetDefinition(number_of_rows=-100, columns=columns)  # Invalid
    ...
    def test_spec_with_generator_options(self):
        ...
        columns = [ColumnDefinition(name="value", type="long")]

        table_def = DatasetDefinition(number_of_rows=100, columns=columns)

        spec = DatagenSpec(
            datasets={"test": table_def}, generator_options={"randomSeedMethod": "hash_fieldname", "verbose": True}
        )
