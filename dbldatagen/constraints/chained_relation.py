...
class ChainedRelation(NoPrepareTransformMixin, Constraint):
    """ChainedRelation Constraint object - constrains one or more columns so that each column has a relationship to the next.

    Constrains one or more columns so that each column has a relationship to the next. For example, if the constraint is defined as `ChainedRelation(['a', 'b','c'], "<")` then only rows that
    satisfy the condition `a < b < c` will be included in the output (where `a`, `b` and `c` represent the data values for the columns).
    ...
    def __init__(self, columns: str | list[str], relation: str) -> None:
