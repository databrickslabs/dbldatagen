...
class Datasets:
    ...
    @classmethod
    def list(cls, pattern: str | None = None, supportsStreaming: bool = False) -> None:
        """
        Lists registered datasets. Optionally filters the list by a regular expression pattern if provided.
    ...
    def describe(cls, name: str) -> None:
        """
        Prints a description for the input dataset.
    ...
    def __init__(self, sparkSession: SparkSession, name: str | None = None, streaming: bool = False) -> None:
        """
        Creates a ``Datasets`` object.

        :param sparkSession: ``SparkSession`` to use
        :param name: Name of the dataset to search for
        :param streaming: If True, validdates that dataset supports streaming
        """
        if not sparkSession:
    ...
    def _getNavigator(self) -> NavigatorNode:
        """
        Gets a navigator for the current dataset.

        :returns: Navigator for the current dataset
        """
    ...
    def _getProviderInstanceAndMetadata(
        ...
        :param providerName: Name of the dataset provider
        :param supportsStreaming: If True, validdates that dataset supports streaming
        :returns: Requested `DatasetProvider` and the associated `DatasetDefinition`
        """
    ...
    def _get(
        self, *, providerName: str, tableName: str | None, rows: int = -1, partitions: int = -1, **kwargs
    ) -> DataGenerator:
        """
        Gets a table generator from the dataset provider.

        :param providerName: Name of the dataset provider
        :param tableName: Optional name of the table to get (if ``None``, the provider's primary table is returned)
        :param rows: Optional number of rows to generate (if -1, provider should compute defaults)
        :param partitions: Optional number of partitions to use (if -1, the number of partitions is computed automatically)
    ...
    def get(self, table: str | None = None, rows: int = -1, partitions: int = -1, **kwargs) -> DataGenerator:
        ...
        These are `DataGenerator` instances that can be used to generate the data. Dataset providers also optionally
        can provide supporting tables which are computed tables based on parameters. These are retrieved using the
        `getAssociatedDataset` method

        If the dataset supports multiple tables, the table may be specified in the `table` parameter.
        If ``None`` is specified, a generator for the primary table is returned.
        ...
        THe default number of rows for each table in a multi-table dataset may vary - for example a 'customers' table
        may have 100,000 rows while a 'sales' table may have 1,000,000 rows.
        ...
        return self._get(providerName=self._name, tableName=table, rows=rows, partitions=partitions, **kwargs)

    def _getSupportingTable(
        self, *, providerName: str, tableName: str, rows: int = -1, partitions: int = -1, **kwargs
    ) -> DataGenerator:
        """
        Gets a supporting table needed to build a multi-table dataset.

        :param providerName: Name of the dataset provider
        :param tableName: Name of the table to get from the provider
        :param rows: Optional number of rows to generate (if -1, provider should compute defaults)
        :param partitions: Optional number of partitions to use (if -1, the number of partitions is computed automatically)
        :param kwargs: Additional keyword arguments to pass to the provider
        :returns: `DataGenerator` for the supporting table
        """
    ...
    def getAssociatedDataset(self, *, table: str, rows: int = -1, partitions: int = -1, **kwargs) -> DataGenerator:
        """
        Gets a table generator from the dataset provider.
        ...
        """
        return self._getSupportingTable(
            providerName=self._name, tableName=table, rows=rows, partitions=partitions, **kwargs
        )
    ...
    def __getattr__(self, path: str) -> NavigatorNode:
...
class NavigatorNode:
    """
    This class is used to navigate datasets and their tables via dotted notation (i.e.
    ``X.dataset_grouping.dataset.table`` where X is an instance of the dataset navigator.)
    ...
    When accessed via dotted notation, the navigator will use the `pathSegment` to locate the provider and create
    it.

    Any remaining `pathSegment` traversed will be used to locate the table within the provider.

    This provides a syntactic layering over the creation of the provider instance and table generation.

    :param datasets: `Datasets` object
    :param providerName: Dataset provider name for the node
    :param tableName: Table name for the node
    :param location: Location for the  node - used in error reporting
    """
    ...
    def _addEntry(
        ...
        :param providerName: Dataset provider name for the node
        :param tableName: Table name for the node
        :returns: `NavigatorNode` with the steps added
        """
        results = self
        if steps is None or len(steps) == 0:
            ...
            if self._children is None:  # no children exist
                newNode = NavigatorNode(datasets, location=new_location)
    ...
    def addEntry(self, datasets: Datasets, providerName: str, tableName: str | None = None) -> None:
        """
        Adds an entry to the dataset navigator.
    ...
    def find(self, attributePath: str) -> NavigatorNode:
        """
        Gets a `NavigatorNode` with the provided path.

        :param attributePath: Attribute path to search
        :returns: `NavigatorNode` with the provided path
        """
    ...
    def isFinal(self) -> bool:
        """
        Checks if the navigator has a named dataset provider.

        :returns: `True` if the navigator has a named dataset provider
        """
    ...
    def __call__(self, *args, **kwargs) -> DataGenerator:
        if not self.isFinal():
