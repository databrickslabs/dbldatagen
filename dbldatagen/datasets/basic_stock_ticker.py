...
@dataset_definition(
    name="basic/stock_ticker", summary="Stock ticker dataset", autoRegister=True, supportsStreaming=True
)
class BasicStockTickerProvider(DatasetProvider.NoAssociatedDatasetsMixin, DatasetProvider):
    ...
    COLUMN_COUNT = 8
    ALLOWED_OPTIONS: ClassVar[list[str]] = ["numSymbols", "startDate"]
    ...
    def getTableGenerator(
        ...
        **options: object,
        ...
        df_spec = (
            ...
            .withColumn("rand_value", "float", minValue=0.0, maxValue=1.0, step=0.1, baseColumn="symbol_id", omit=True)
            .withColumn(
                "symbol",
                "string",
            ...
            .withColumn(
                "start_value",
                "decimal(11,2)",
                values=[1.0 + 199.0 * random() for _ in range(max(1, int(numSymbols / 10)))],
                omit=True,
            )
            .withColumn(
                "growth_rate",
                "float",
                values=[-0.1 + 0.35 * random() for _ in range(max(1, int(numSymbols / 10)))],
                baseColumn="symbol_id",
            )
            .withColumn(
                "volatility",
                "float",
                values=[0.0075 * random() for _ in range(max(1, int(numSymbols / 10)))],
                baseColumn="symbol_id",
                omit=True,
            )
            .withColumn(
            ...
            .withColumn("modifier_sign", "float", expr="case when sin(id % 17) > 0 then -1.0 else 1.0 end", omit=True)
            .withColumn(
                "open_base",
                "decimal(11,2)",
                expr=f"""start_value
