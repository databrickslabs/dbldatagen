...
# MAGIC
# MAGIC This notebook demonstrates how to use [**dbldatagen**](https://databrickslabs.github.io/dbldatagen/public_docs/index.html) to simulate data from a supply chain for consumer packaged goods (CPG).
# MAGIC
# MAGIC
...
# MAGIC 4. **Orders** - Manufacturing execution data
...
# MAGIC
# MAGIC dbldatagen can be installed using pip install commands, as a cluster-scoped library, or as a serverless environment-scoped library.
...
products_spec = (
    ...
    .withColumn("created_date", "date", begin="2020-01-01", 
               end="2024-01-01", interval="1 day", random=True )
)
...
distribution_center_spec = (
    dg.DataGenerator(spark, name="distribution_centers", rows=NUM_DISTRIBUTION_CENTERS, partitions=4)
    .withIdOutput()
    
    # distribution_center codes: distribution_center-0001, distribution_center-0002, etc.
    .withColumn("distribution_center_code", "string", 
                expr="concat('distribution_center-', lpad(cast(id as string), 4, '0'))", 
                uniqueValues=NUM_DISTRIBUTION_CENTERS)
    
    .withColumn("distribution_center_name", "string", template=r"\\w Distribution Center")
...
# MAGIC
# MAGIC ### Learning Objectives:
# MAGIC - Creating foreign key relationships (distribution_center_id references distribution_centers)
# MAGIC - Generating realistic store attributes
# MAGIC - Using longer store codes (6 digits vs 4 for distribution_centers)
...
stores_spec = (
    ...
    # Each store gets a distribution_center ID between 1 and NUM_DISTRIBUTION_CENTERS
    .withColumn("distribution_center_id", "integer", minValue=1, maxValue=NUM_DISTRIBUTION_CENTERS, random=True)
...
order_spec = (
    dg.DataGenerator(spark, name="orders", rows=NUM_ORDERS, partitions=8)
    .withIdOutput()
...
# MAGIC To get 30% distribution_center and 70% Store:
# MAGIC 1. Create a seed column with values 0-1
# MAGIC 2. Use CASE: when seed < 0.3 then 'distribution_center' else 'Store'
# MAGIC
...
inventory_spec = (
    ...
    # Create location_id based on location_type using expr
    ...
    # Calculate reserved quantity using expr
    .withColumn("quantity_reserved", "integer", expr="cast(quantity_on_hand * reserve_factor as int)")
    
    # Calculate available quantity
    .withColumn("quantity_available", "integer", expr="quantity_on_hand - quantity_reserved")
    ...
    # Calculate days of supply with safe division
...
# Build and drop intermediate columns
...
shipments_spec = (
    ...
    # FOREIGN KEY: Origin is always a distribution_center
    .withColumn("origin_distribution_center_id", "integer", minValue=1, maxValue=NUM_DISTRIBUTION_CENTERS, random=True)
    
    # Destination can be distribution_center or Store (30% distribution_center, 70% Store)
    ...
    # Create destination_id based on destination_type
    ...
    # Expected delivery = ship_date + transit_days (using date_add)
    .withColumn("expected_delivery", "timestamp", expr="date_add(ship_date, transit_days)")
...
# MAGIC %sql
# MAGIC -- distribution_center performance and utilization
# MAGIC SELECT 
# MAGIC   distribution_center.distribution_center_code,
# MAGIC   distribution_center.region,
# MAGIC   distribution_center.capacity_pallets,
# MAGIC   ROUND(distribution_center.current_utilization_pct, 1) as utilization_pct,
...
# MAGIC FROM distribution_centers distribution_center
# MAGIC LEFT JOIN inventory i ON distribution_center.id = i.location_id 
# MAGIC   AND i.location_type = 'distribution_center'
# MAGIC   AND i.snapshot_date = (SELECT MAX(snapshot_date) FROM inventory)
# MAGIC LEFT JOIN shipments s ON distribution_center.id = s.origin_distribution_center_id
