...
testdata_generator = (
    dg.DataGenerator(spark, name="test_dataset1", rows=100000, partitions=20, randomSeedMethod="hash_fieldname")
...
# will have implied column `id` for ordinal of row
testdata_generator2 = (
    dg.DataGenerator(spark, name="test_dataset2", rows=100000, partitions=20, randomSeedMethod="hash_fieldname")
    .withIdOutput()  # id column will be emitted in the output
    .withColumn("code1", "integer", minValue=1, maxValue=20, step=1)
    .withColumn("code2", "integer", minValue=1, maxValue=20, step=1, random=True)
    .withColumn("code3", "integer", minValue=1, maxValue=20, step=1, random=True)
    ...
    .withColumn("site_cd", "string", prefix='site', baseColumn='code1')
    .withColumn("sector_status_desc", "string", minValue=1, maxValue=200, step=1, prefix='status', random=True)
    .withColumn("tech", "string", values=["GSM", "UMTS", "LTE", "UNKNOWN"], weights=[5, 1, 1, 1], random=True)
    .withColumn("test_cell_flg", "integer", values=[0, 1], random=True)
)
...
spark.sql(
    """Create table if not exists test_vehicle_data(
                name string, 
...
dataspec = dg.DataGenerator(spark, rows=10000000, partitions=8, randomSeedMethod="hash_fieldname").withSchema(
    table_schema
)
...
testDataSpec = (
    ...
    .withColumn("country", StringType(), values=country_codes, weights=country_weights, baseColumn="internal_device_id")
    .withColumn("manufacturer", StringType(), values=manufacturers, baseColumn="internal_device_id")
    # use omit = True if you don't want a column to appear in the final output
    # but just want to use it as part of generation of another column
    .withColumn("line", StringType(), values=lines, baseColumn="manufacturer", baseColumnType="hash", omit=True)
    .withColumn(
        "model_ser", IntegerType(), minValue=1, maxValue=11, baseColumn="device_id", baseColumnType="hash", omit=True
    )
