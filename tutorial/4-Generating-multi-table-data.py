...
customer_dataspec = (
    ...
    .withColumn("device_id", "decimal(10)", minValue=DEVICE_MIN_VALUE, baseColumn="customer_id", baseColumnType="hash")
    .withColumn(
        "phone_number",
        "decimal(10)",
        minValue=SUBSCRIBER_NUM_MIN_VALUE,
...
events_dataspec = (
    dg.DataGenerator(
        spark, rows=data_rows, partitions=partitions_requested, randomSeed=42, randomSeedMethod="hash_fieldname"
    )
    # use same logic as per customers dataset to ensure matching keys - but make them random
    .withColumn(
        "device_id_base",
        "decimal(10)",
        minValue=CUSTOMER_MIN_VALUE,
        uniqueValues=UNIQUE_CUSTOMERS,
        random=True,
        omit=True,
    )
    .withColumn(
        "device_id", "decimal(10)", minValue=DEVICE_MIN_VALUE, baseColumn="device_id_base", baseColumnType="hash"
    )
...
df_enriched_events = (
    ...
    .withColumn(
        "ld_minutes", F.expr("case when event_type='ld call' then cast(ceil(minutes) as decimal(18,3)) else 0.0 end")
    )
    .withColumn(
        "local_minutes",
        F.expr("case when event_type='local call' then cast(ceil(minutes) as decimal(18,3)) else 0.0 end"),
    )
    .withColumn(
        "intl_minutes",
        F.expr("case when event_type='intl call' then cast(ceil(minutes) as decimal(18,3)) else 0.0 end"),
    )
)
...
df_customer_summary = df_customer_pricing.join(
