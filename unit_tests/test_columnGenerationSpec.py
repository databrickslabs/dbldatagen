...
class TestColumnGenerationSpec(unittest.TestCase):
    def test_getNames(self):
        cd = datagen.ColumnGenerationSpec(name="test")
        results = cd.getNames()
        self.assertEqual(results, ['test'])

    def test_getNames2(self):
        cd = datagen.ColumnGenerationSpec(name="test", numColumns=3)
        results = cd.getNames()
        self.assertEqual(results, ['test_0', 'test_1', 'test_2'])

    def test_isFieldOmitted(self):
        cd = datagen.ColumnGenerationSpec(name="test", omit=True)
        self.assertTrue(cd.omit)

    def test_colType(self):
        dt = StringType()
        cd = datagen.ColumnGenerationSpec(name="test", colType=StringType())
        self.assertEqual(cd.datatype, dt)

    def test_baseColumn(self):
        dt = StringType()
        cd = datagen.ColumnGenerationSpec(name="test", colType=StringType(), base_column='test0')
