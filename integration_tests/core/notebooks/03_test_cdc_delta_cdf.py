...
# MAGIC %md
# MAGIC # 03 — Test CDC Delta CDF Format
# MAGIC
# MAGIC Generates CDC stream with `format="delta_cdf"` (10 batches) and verifies
# MAGIC CDF-specific columns, initial snapshot integrity, and format correctness.
...
TABLE_PREFIX = f"{PREFIX}cdc_cdf_"
TEST_NAME = "cdc_delta_cdf"
...
# MAGIC %md
# MAGIC ## Generate CDC Stream (delta_cdf format)

# COMMAND ----------

plan = build_employee_plan()
start = time.time()
stream = generate_cdc(spark, plan, num_batches=NUM_BATCHES, format="delta_cdf")
gen_time = time.time() - start
print(f"CDC delta_cdf generation (lazy) took {gen_time:.1f}s")
...
# Initial snapshot row count checks (commit_version == 0)
...
# CDF format checks
CDF_REQUIRED_COLS = {"_change_type", "_commit_version", "_commit_timestamp"}
RAW_COLS = {"_op", "_batch_id"}
VALID_CHANGE_TYPES = {"insert", "update_preimage", "update_postimage", "delete"}
