...
# MAGIC %md
# MAGIC # 02 — Test CDC Raw Format
# MAGIC
# MAGIC Generates CDC stream with `format="raw"` (10 batches) and verifies
# MAGIC initial snapshot integrity, batch structure, and operation types.
...
TABLE_PREFIX = f"{PREFIX}cdc_raw_"
TEST_NAME = "cdc_raw"
...
for table_name in EXPECTED_ROWS:
    ...
    # Count distinct batch_ids (should be 0..10 = 11 total)
    batch_ids = sorted([
        r["_batch_id"] for r in full_df.select("_batch_id").distinct().collect()
    ])
    num_batches_found = len([b for b in batch_ids if b > 0])
    passed_batches = num_batches_found == NUM_BATCHES
    summary_rows.append((
        TEST_NAME, table_name, "batch_count", num_batches_found, NUM_BATCHES,
        passed_batches,
        json.dumps({"batch_ids": batch_ids}),
        datetime.now(),
    ))
    ...
    batch_counts = {
        r["_batch_id"]: r["count"]
        for r in full_df.groupBy("_batch_id").count().collect()
    }
    empty_batches = [b for b in range(1, NUM_BATCHES + 1) if batch_counts.get(b, 0) == 0]
    passed_nonempty = len(empty_batches) == 0
    summary_rows.append((
        TEST_NAME, table_name, "batches_nonempty",
        len(batch_counts) - 1, NUM_BATCHES, passed_nonempty,
        json.dumps({"empty_batches": empty_batches, "batch_counts": {str(k): v for k, v in batch_counts.items()}}),
        datetime.now(),
    ))

    # Check _op values (only non-initial rows)
    change_rows = full_df.filter(F.col("_batch_id") > 0)
    if change_rows.count() > 0:
        op_values = sorted([r["_op"] for r in change_rows.select("_op").distinct().collect()])
        valid_ops = {"I", "U", "UB", "D"}
        invalid_ops = [op for op in op_values if op not in valid_ops]
        passed_ops = len(invalid_ops) == 0
    else:
        op_values = []
        passed_ops = True
    summary_rows.append((
        TEST_NAME, table_name, "ops_valid", len(op_values), 0, passed_ops,
        json.dumps({"op_values": op_values, "valid_ops": sorted(valid_ops)}),
        datetime.now(),
    ))
    print(f"  {table_name}: batches={num_batches_found}, ops={op_values}")
